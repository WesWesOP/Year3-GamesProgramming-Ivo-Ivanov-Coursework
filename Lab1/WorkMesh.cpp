...
void WorkMesh::init(Vertex* vertices, unsigned int numVertices, unsigned int* indices, unsigned int numIndices)
{
	IndexedModel model;
...
void WorkMesh::initModel(const IndexedModel& model)
{

	countDraw = model.indices.size();
	...
	glBindBuffer(GL_ARRAY_BUFFER, vertexArrayBuff[TEXCOORDINATES_VB]); //tell opengl what type of data the buffer is (GL_ARRAY_BUFFER), and pass the data
...
void WorkMesh::loadModel(const std::string& filename)
{
...
void WorkMesh::Make()
{
	...
	glDrawElements(GL_TRIANGLES, countDraw, GL_UNSIGNED_INT, 0);
	glDrawArrays(GL_TRIANGLES, 0, countDraw);

	glBindVertexArray(0);
}

void WorkMesh::updateSphereData(glm::vec3 pos, float radius)
{
	sphereMesh.SetPosition(pos);
