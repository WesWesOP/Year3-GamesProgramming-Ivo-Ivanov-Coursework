...
Audio::Audio()
{
	device = alcOpenDevice(NULL); //open sound card in AL space

	if (device == NULL)
	{
		std::cout << "cannot open sound card" << std::endl; //print out message if device returns NULL
	}

	context = alcCreateContext(device, NULL); //open context in AL space

	if (context == NULL)
	{
		std::cout << "cannot open context" << std::endl; //print out message of context returns NULL
	}
